...
@Component({
    styleUrls: ['askcatie.component.scss'],
    templateUrl: 'askcatie.component.html',
})

export class AskCATieComponent implements OnInit, OnDestroy {
    ...
    constructor(private fb: FormBuilder, private askCATieService: AskCATieService, private router: Router,
    ...
    ngOnInit() {
        ...
        this.askCATieService.getAllQuestions().subscribe(
            ...
            function() { console.log("completed askcatie loading"); }
        );
    }
    ...
    getQuestion(questionTitle: string) {
        ...
        this.questionSubscription = this.askCATieService.getQuestion(this.selectedQuestion.course, this.selectedQuestion.lecture,
    ...
    addComment() {
        if (this.parentId === 0) {
            let status = this.askCATieService.addComment(this.model.commentBody, this.selectedQuestion.id,
            ...
            console.log("status on AskCATie page: ");
            console.log(status);
        } else {
            let status = this.askCATieService.addCommentReply(this.model.commentBody, this.selectedQuestion.id,
    ...
    createQuestion() {
        this.askCATieService.createQuestion(this.model.title, this.selectedCourse.code,
    ...
    upvoteComment(commentId: number, isUpvote: boolean) {
        ...
        this.askCATieService.upvoteComment(code, lecture, questionId, commentId, isUpvote).subscribe(status => {
            // Update display
            this.getQuestion(this.selectedQuestion.title);
        });
    }
    ...
    // Filters questions
    onCourseChange() {
        if (this.selectedCourse == null) {this.askCATieService.getAllQuestions().subscribe(
        ...
        this.getLectures();
        this.askCATieService.getQuestionsForCourse(courseCode).subscribe(
            questions => this.questions = questions,
            function(error) { console.log(error); },
    ...
    onLectureChange() {
        ...
        this.askCATieService.getQuestionsForCourseAndLecture(courseCode, lectureUrlName).subscribe(
            questions => this.questions = questions,
            function(error) { console.log(error); },
